...
// Assignment for ERC-20 Token Standard
import "./safemath.sol";
...
contract Token {
  ...
  function approve(address _spender,uint256 _value) public returns (bool _success) {
    require(balances[msg.sender] >= _value, "Not enough tokens"); // not required here, I might set I value to be used in the future (requirements set in transferFrom)
    // may require not allowing same address as msg.sender, or not allowing a 0 address (not a real address)
  ...
  function transferFrom(address _from,address _to,uint256 _value) public returns (bool _success){
    ...
    allowed[_from][msg.sender] = allowed[_from][msg.sender].sub(_value); //allowed tokens to be spent should also decrease
    balances[_to] = balances[_to].add(_value);
